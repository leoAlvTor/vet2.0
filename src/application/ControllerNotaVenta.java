...
public class ControllerNotaVenta {
    ...
    private void init() {
        ...
        txtNumFac.setText(dbQueries.getNotaActual());
    }
    ...
    private void guardarDatosCabecera() {
        ...
        dbInserts.connect();
        if(radioContado.isSelected()) {
            dbInserts.insertNota(clienteSeleccinado.getCi(), "contado", txtFecha.getText(), txtSubtotal.getText(),
                txtIVA.getText(),
                    txtDescuento.getText(), txtTotal.getText(), empleado);
        }else if(radioCredito.isSelected()) {
            dbInserts.insertNota(clienteSeleccinado.getCi(), "credito", txtFecha.getText(), txtSubtotal.getText(),
                txtIVA.getText(),
                    txtDescuento.getText(), txtTotal.getText(), empleado);
        }
    ...
    private void guardarFacturaDetalle(DBInserts dbInserts) {
        for(FacturaDetalle p : tblDatos.getItems())
            dbInserts.insertNDet(txtNumFac.getText(), p.getCodigo(), p.getDescripcion(), p.getTarifa(), p.getCantidad(),
